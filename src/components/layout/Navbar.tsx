    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 w-full z-50 backdrop-blur-md border-b transition-all duration-300
          ${scrolled ? "bg-black/80 py-3 border-[#2a2a2a]" : "bg-black/40 py-5 border-transparent"}`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link href="/cart" className="relative">
              ...
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 text-[10px] rounded-full bg-blue-600 px-1.5 py-0.5">
                  {totalItems}
                </span>
              )}
            </Link>
